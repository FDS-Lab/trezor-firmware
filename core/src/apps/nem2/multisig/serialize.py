...
from trezor.crypto import base32, random
from trezor.messages.NEM2MultisigModificationTransaction import (
...
def get_multisig_modification_body_size(
    multisig_modification: NEM2MultisigModificationTransaction,
):
...
def serialize_multisig_modification_body(
    multisig_modification: NEM2MultisigModificationTransaction,
) -> bytearray:
    ...
    write_uint8(
        tx,
        unsigned_32_bit_int_to_8_bit(
            multisig_modification.min_removal_delta, signed=False
        ),
    )
    ...
    write_uint8(
        tx,
        unsigned_32_bit_int_to_8_bit(
            multisig_modification.min_approval_delta, signed=False
        ),
    )
