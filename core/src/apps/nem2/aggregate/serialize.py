...
def serialize_according_to_type(transaction):
    ...
    return serialize_function(
        transaction.common, transaction.__dict__[transaction_type_key], embedded=True
    )
...
def compute_inner_transaction_hash(transactions: NEM2InnerTransaction):
    mt = MerkleTools()
...
def serialize_inner_transactions(inner_transactions: NEM2InnerTransaction):
    txs = bytearray()
    for transaction in inner_transactions:
        serialized_transaction = serialize_according_to_type(transaction)
        inner_transaction_padding = getInnerTransactionPaddingSize(
            len(serialized_transaction), 8
        )
...
def serialize_cosignatures(cosignatures: NEM2Cosignature):
    txs = bytearray()
...
def serialize_aggregate_transaction_body(
    inner_transaction_hash, serialized_inner_transactions, cosignatures=bytearray()
):
    tx_body = bytearray()
...
def serialize_aggregate_transaction(
    common: NEM2TransactionCommon, aggregate: NEM2AggregateTransaction
):
    tx = bytearray()

    # Transactions hash
    inner_transaction_hash = compute_inner_transaction_hash(
        aggregate.inner_transactions
    )
    # Serialize the inner trascations
    serialized_inner_transactions = serialize_inner_transactions(
        aggregate.inner_transactions
    )
    # Serialize the cosignatures
    serialized_cosignatures = serialize_cosignatures(aggregate.cosignatures)
    # Serialize the body
    serialized_body = serialize_aggregate_transaction_body(
        inner_transaction_hash, serialized_inner_transactions, serialized_cosignatures
    )
