...
def validate(msg: NEM2SignTx):
    ...
    if msg.namespace_registration:
        _validate_namespace_registration(
            msg.namespace_registration, msg.transaction.version
        )
...
def address_validator(address_data):
    ...
    if first_char.upper() == "S":
        if network_type != NEM2_NETWORK_MIJIN_TEST:
            raise ProcessError(
                "Network type for address is invalid. Must be Mijin Test"
            )
...
def validate_secret(secret, hash_algorithm):
    if (
        ...
        or hash_algorithm == NEM2_SECRET_LOCK_KECCAK_256
        or hash_algorithm == NEM2_SECRET_LOCK_HASH_256
    ):
        if len(secret) != 64:
            raise ProcessError(
                "Secret must be of length 64 (was {})".format(len(secret))
            )
    elif hash_algorithm == NEM2_SECRET_LOCK_HASH_160:
        if len(secret) != 40 or len(secret) != 64:
            raise ProcessError(
                "Secret must be of length 40 or 64 (was {})".format(len(secret))
            )
    else:
        raise ProcessError("Invalid hash algorithm selected")


def _validate_account_address_restriction(
...
def _validate_account_mosaic_restriction(
...
def _validate_account_operation_restriction(
...
def _validate_account_link(account_link: NEM2AccountLinkTransaction):
    ...
    valid_actions = [NEM2_ACCOUNT_LINK_ACTION_UNLINK, NEM2_ACCOUNT_LINK_ACTION_LINK]
...
def _validate_mosaic_global_restriction(
    global_restriction: NEM2MosaicGlobalRestriction,
):
...
def _validate_mosaic_address_restriction(
    address_restriction: NEM2MosaicAddressRestriction,
):
