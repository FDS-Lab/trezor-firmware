...
from trezor.messages.NEM2NamespaceRegistrationTransaction import (
    NEM2NamespaceRegistrationTransaction,
)
...
def _validate_namespace_registration(
    namespace_registration: NEM2NamespaceRegistrationTransaction, version: int
):
    if (
        namespace_registration.registration_type
        != NEM2_NAMESPACE_REGISTRATION_TYPE_ROOT
        and namespace_registration.registration_type
        != NEM2_NAMESPACE_REGISTRATION_TYPE_SUB
    ):
    ...
    if (
        ...
        if (
            int(namespace_registration.duration) < 1
            or int(namespace_registration.duration) > 2102400
        ):
...
def _validate_address_alias(address_alias: NEM2AddressAliasTransaction, version: int):
    if (
        address_alias.alias_action != NEM2_ALIAS_ACTION_TYPE_LINK
        and address_alias.alias_action != NEM2_ALIAS_ACTION_TYPE_UNLINK
    ):
